...
That last one is why command structs hold **raw strings** (`Amount string`, `Date string`) rather than parsed types. This looks wrong, it is deliberate, and [ADR-0005](decisions/0005-shared-application-layer.md) is the long answer. The short one: if the CLI parses a date and the API parses a date, they will eventually disagree about what "today" means at a month boundary — so neither of them parses dates.
