...
The [`Makefile`](../Makefile) is the build contract. **Every PR runs `make check-go` and `make check-web`, and nothing else** — if a check isn't reachable through a `make` target, it isn't part of the build, and there is no divergent CI-only command list to keep in sync. Each is gated in CI ([`ci.yml`](../.github/workflows/ci.yml)) on whether a commit actually touched that side ([`dorny/paths-filter`](https://github.com/dorny/paths-filter)) — a web-only PR doesn't pay for a Go job and vice versa. `make check` runs both unconditionally; run that locally before pushing. There is no separate `make build` job on the PR path: `check-go`'s Go tests already compile `cmd/bodger`, and whenever `check-web` runs, its e2e test already does a real `npm run build` + `go build` of the production binary and runs it — a stronger check than an unrun `make build` compile. A push to `main` (post-merge) runs a separate, much lighter workflow instead ([`build-main.yml`](../.github/workflows/build-main.yml)): just `make build`, no lint or tests — cheap insurance against a merge that applies cleanly in git but doesn't actually compile once combined with whatever else landed on main first, not a repeat of the PR's own checks.
...
**Surface-facing vs. internal-only: the raw-string rule is about who constructs the struct, not what it's named.** Most `internal/app` structs exist so a surface can build one from decoded transport input — those always take raw strings, no exceptions. A few exist purely for `internal/app` to call itself with a domain value it already has in hand: `ConvertAmountQuery.Amount` is a real `money.Money`, not a string, because its only two callers (`list_fx_rates.go`, `balances.go`) already hold one — a stored balance, or an amount built two lines up from a string that was just parsed — so a string field there would mean parsing back something never rendered as text in the first place. That's a legitimate exception, not a loophole: check who actually constructs the struct before typing a field this way, and if the answer is ever "a surface, directly," it needs a raw string, however much it resembles an internal-only query. `ListFxRatesQuery.Amount` was typed `*money.Money` for exactly this reason once — copied from `ConvertAmountQuery`'s shape without noticing `ListFxRates` has no internal caller with a `Money` in hand, only CLI/HTTP ones with a typed string on a flag or query param and no way to build one (barred from `internal/domain` by the import-graph check below). Fixed by making it a plain string like everything else in this section — the exception was never earned in the first place.
...
- **Anything the web UI renders is held to [`design-system.md`](design-system.md)** — color, type, spacing, radius, and which `web/src/components/ui` primitive to reach for. Add or refresh a primitive with the shadcn CLI (`cd web && npx shadcn@latest add <component>`, `--overwrite` to refresh) — a hard requirement, not a suggestion; never hand-roll or hand-edit one that the registry already owns. `--overwrite` can silently drop a local fix layered onto that primitive, so always diff the result before committing — see [`design-system.md`'s Component primitives section](design-system.md#component-primitives) for that caveat and a real gotcha it caught.
