...
- **Vocabulary discipline needs enforcement it does not yet have.** [ADR-0005](0005-shared-application-layer.md)'s conformance suite checks that surfaces *behave* identically; nothing yet checks that they *speak* the same way. A banned-term lint over user-facing strings is the obvious mechanism and is filed as follow-up work, not built.
