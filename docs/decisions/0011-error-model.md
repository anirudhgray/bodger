...
- **It takes discipline, now mechanically enforced.** `fmt.Errorf` is right there, and it will get used. `internal/lint` ([#12](https://github.com/anirudhgray/bodger/issues/12)) fails `make check` on a `fmt.Errorf` or `errors.New` in `internal/app` that isn't nested inside a `Wrap(...)` call — the boundary, not a ban: a driver error becoming a cause is exactly right. It parses rather than type-checks, so it matches the method name `Wrap` and takes `internal/domain` and the adapters as out of scope by construction; a `Wrap` on some other type would silence it. Upgrade to a `go/analysis` pass if that ever costs anything real.
