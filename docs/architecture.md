...
[#282](https://github.com/anirudhgray/bodger/issues/282) lands the milestone's last piece, the web UI, on top of #281's/#294's REST surface: a **Recurring** sidebar entry (`web/src/pages/Recurring.tsx`) with two structurally separate sections in the same "Budgets-screen parity" interaction shape #245 established — a `Rules` card (create/edit/archive via `RecurringRuleDialog.tsx`, which fixes account/category/starts-on as read-only once a rule exists, mirroring `PATCH .../recurring-rules/{id}`'s own contract) and a second, dashed-border/muted-background `Upcoming (projected)` card listing every pending occurrence with its own `Projected` badge and Materialise/Skip actions — a separate, visually distinct section rather than a status badge on an otherwise-ordinary row, per data-model.md §11's "always visually and structurally distinguished from actuals." Refresh is a single, explicit, page-level "Refresh occurrences" button (`POST .../recurring-occurrences/refresh` with no `rule_id`, refreshing every active rule at once) rather than an automatic on-page-load call — ADR-0014 requires generation to stay "an explicit... action a surface took," and a button makes that choice visible rather than hiding a write behind a navigation. `pages/Analytics.tsx` gained a `Forecast (projected)` card (`GET /api/v1/forecast`, filtered only by an opt-in "Forecast through" date — "from" is never asked for, since an omitted date resolves to today server-side, the same normalize-once rule every other date field here follows): its own dashed/muted styling and `Projected inflow`/`Projected outflow` labels are this screen's version of the same derived-number distinction, applied as a separate always-visible card rather than Balances' click-to-expand rate-provenance pattern, since a forecast has no separate "real" figure underneath it to disclose from — the forecast *is* the derived figure. `web/e2e/recurring.spec.ts` covers the full flow (create a rule, refresh, see a projected occurrence, materialise it, see it become a real transaction on `/transactions`) against a dedicated account created within the spec itself, isolated from `smoke.spec.ts`'s own opening-balance arithmetic on the shared seeded account — the two specs share one server/DB (`playwright.config.ts`'s `workers: 1`), so a real money-moving materialise call in one spec would otherwise leak into another's balance assertions.

**M10 · Valinor is complete.**
[ADR-0015](decisions/0015-ai-assisted-suggestions.md) ([#298](https://github.com/anirudhgray/bodger/issues/298)) settles the design before anything is built against it, following M8's and M9's sequencing. The decisions worth knowing before reading any M10 code: the feature is **off unless `BODGER_TYPESAFE_API_KEY` is set**, so an instance that ignores the milestone is unchanged in behaviour, dependencies, and privacy; suggestions are **advisory and never persisted**, produced by one new read-only app method (`SuggestForImportBatch`) that writes nothing, with every subsequent write going through an existing human-confirmed use case; and **confidence never auto-applies at any value** — it decides only what is shown, since the 0.5 cutoff is the vendor's own band boundary rather than anything calibrated against bodger's data, and a writing threshold would silently change meaning when the vendor moves a model alias (hence the pinned `jev-1.13.0`). One port, `ports.SuggestionProvider`, defined in bodger's own vocabulary rather than the vendor's Choice/Score/Noul primitives, keeps the provider replaceable — including by a local classifier later, once enough confirmed categorisations exist to train one. On the surfaces, every suggestion is **attributed to `typesafe.ai` by name** at the point someone acts on it, inheriting the transparency rule [`ux-principles.md`](ux-principles.md) §6 already set for FX (`source: frankfurter` next to a converted amount) rather than inventing a softer one for AI — and an instance with no key says so once, quietly, where the suggestion would have been, since otherwise a deliberate privacy choice is indistinguishable from the feature being broken.

The rule that shapes the code, though, is **"code answers the arithmetic; the model answers only the semantics."** typesafe.ai's own guidance is that code should own the workflow and the model should get narrow structured decisions, and its published per-version limitations page for `jev-1.13` states that the model "reads dates as text, not as ordered quantities" and "is not a calculator". Occurrence matching is fundamentally a date-window and amount comparison, so that half is done deterministically in `internal/app` first — reusing `findSuspectedDuplicate`'s existing same-currency/same-amount/in-window logic — and the model is only ever asked which of the *already eligible* candidates a row's description refers to. Categorisation gets the same treatment: the row's amount sign decides expense-vs-income in code, so the option set is filtered by `CategoryKind` before any request is built. For the same reason the unit of request is **one row**, not a batch of them: the vendor's batching win comes from many questions about one document, whereas staged rows are independent records, and a 200-row `state` would make every question mostly distractor content ("accuracy falls as the state grows with content unrelated to the decision"). Throughput comes from a bounded concurrent worker pool in the adapter instead, and the per-row shape is the more private one too.

Two things the ADR changed rather than recorded. **The milestone's second use case was re-scoped**: §8 and #298 both described a suggestion on "the recurring-rule matching path", but no such path exists — `internal/app/recurring_rules.go` is CRUD only, and ADR-0014 has no concept of matching a transaction to a rule. The real gap is that `findSuspectedDuplicate` (`internal/app/import_duplicate.go`) searches committed transactions only, so an imported row that duplicates a **pending `ScheduledOccurrence`** is invisible to duplicate detection — a genuine pre-existing bug, tracked separately as [#301](https://github.com/anirudhgray/bodger/issues/301), and the reason the second use case is now occurrence matching at the same import-review call site, where `MaterialiseOccurrence`/`SkipOccurrence` already provide the confirm step. **And the privacy posture is now an explicit, documented boundary**: the provider receives a minimal `SuggestionRow` projection, never an `importing.ImportRecord`, whose `RawPayload` carries the entire original CSV line — account numbers and running balances included — and an adapter test pins that it never reaches the wire.

The foundation ([#303](https://github.com/anirudhgray/bodger/issues/303): config, `ports.SuggestionProvider`, service wiring), the adapter ([#304](https://github.com/anirudhgray/bodger/issues/304): `internal/adapters/typesafe`), and #301's fix are merged. [#305](https://github.com/anirudhgray/bodger/issues/305) lands the app-layer method the rest of this section describes: `Service.SuggestForImportBatch` (`internal/app/import_suggest.go`), capped at 200 rows a run so a large CSV can't become an unbounded spend from one click, and its own 254-option ceiling per Choice (refuses a category question rather than truncating the option set when an actor's category list exceeds it — a truncated set risks silently excluding the correct answer). Its occurrence-candidate narrowing reuses `findOccurrenceMatch`'s own currency/amount/date-window comparison via a new shared `eligibleOccurrences` helper (`import_duplicate.go`) rather than a second copy of it — `findOccurrenceMatch` now layers its `descriptionsSimilar` filter on top of that shared candidate list instead of re-deriving it.

[#306](https://github.com/anirudhgray/bodger/issues/306) wires `SuggestForImportBatch` onto all three in-process surfaces, following #281's own recurring-surface precedent for shape: `GET /api/v1/imports/{id}/suggestions` (REST, read-only — nothing under `/api/v1/imports` gained a write here), `bodger import suggest <import-id>` (CLI, under the existing `import` group), and `get_import_suggestions` (MCP, read tier — the method writes nothing, so it needs neither the confirmation-token protocol nor `--allow-destructive`), plus CLI/HTTP/MCP conformance rows proving all three render the unconfigured, configured-with-suggestions, and provider-failure states identically. Every suggestion is attributed to `typesafe.ai` by a literal string constant on each surface, not a value read off the result: `ports.SuggestionProvider` deliberately carries no `Name()` (#305's own doc comment already flagged this — nothing it returns is persisted, so there's no provenance column to fill), and an instance has only one configured provider for a suggestion to have come from. The same gap means no model identifier reaches any surface either; `--json`/the REST response disclose confidence (never a raw decimal in CLI human output, per `ux-principles.md` §6's progressive disclosure), but not a model version, since nothing between the adapter and the app layer carries one today — a real, if minor, gap this issue flags rather than papering over by inventing one. A second, sharper gap: `SuggestForImportBatchResult` only ever exposes an aggregate `RowsFailed int`, not ADR-0015's own credential-rejected/throttled/provider-unreachable distinction — `SuggestForImportBatch` deliberately discards `ports.SuggestionProvider.Suggest`'s error into that count (its own doc comment: "a failed suggestion run is worth reporting, not worth blocking review over"), so no surface can currently tell an operator *why* a row got no answer, only that some rows didn't. Every surface still keeps every staged row fully listable regardless (verified by a provider-failure conformance case), which is the acceptance criterion ADR-0015 actually requires; distinguishing the reason is real follow-up work, not silently dropped scope. That follow-up has since landed, as a small prerequisite PR ahead of #307: `ports.SuggestionProvider.Suggest` now returns a third value, `ports.SuggestOutcome{FailedRows, FailureReason}`, alongside its existing slice and error — a new signal rather than a repurposing of the existing "error only on total failure" contract, so a partial failure's reason survives instead of being observed by the adapter and then dropped. `SuggestForImportBatch` threads it onto a new `SuggestForImportBatchResult.FailureReason` field, and all three surfaces render it: `failure_reason` on the REST response and the CLI's `--json` output carries the raw reason string (`credential_rejected`/`throttled`/`provider_unreachable`), the CLI's human-readable table translates it into a plain-English clause, and the MCP tool result carries the same raw string a REST caller would see. It is one dominant reason for the whole `SuggestForImportBatch` call, not tracked per row — every row in one call shares the same instance credential and the same provider, so a real failure produces the same reason on essentially every failed row, and the adapter's own doc comments say so explicitly rather than hiding the simplification. This issue also found and fixed a genuine wiring gap outside its own files' boundary: `cmd/bodger/main.go`'s `bootstrap` never set `Service.Suggestions` from `config.TypesafeAPIKey` at all, so setting the environment variable had no effect on the real binary regardless of what #303/#304 shipped — `bootstrap` now constructs `typesafe.New` and wires it in only when the key is non-empty, leaving `Suggestions` `nil` (and the binary starting cleanly with no outbound call) otherwise.

[#307](https://github.com/anirudhgray/bodger/issues/307) lands the milestone's last piece, the web UI, on top of #306's REST surface — extending `web/src/pages/data/Import.tsx`'s existing `ReviewStep`, the staged-rows table #214 already built around `duplicate_match` resolution. `getImportSuggestions` (`web/src/lib/api.ts`) is called once when the review step opens for a batch (not on every render), never on upload or mapping, so a metered call is only ever made for a batch someone is actually reviewing. Suggestions render per staged row in a new "Suggested" column, ranked exactly as the REST response returns them (no client-side re-sort) and with no raw confidence decimal anywhere — the response's own ordering is the only use this screen makes of confidence. Every suggestion names `typesafe.ai` plainly, mirroring `Balances.tsx`'s `source: frankfurter` attribution treatment rather than inventing a softer one for AI. The two suggestion kinds are visually distinct, per the issue's own acceptance criterion: a category suggestion is a plain solid-border card with no accept control, and an occurrence suggestion gets `Recurring.tsx`'s own dashed-border, muted-background "Projected" visual language (data-model.md §11) plus a real "Accept match" button. The not-configured and failed states each render as one quiet, non-blocking line where the suggestion column would otherwise show content — never a modal or a banner, and every staged row stays fully listable and reviewable either way (`explainSuggestionFailure` in `Import.tsx` mirrors the CLI's own `explainFailureReason` translator, reimplemented rather than shared since the CLI's Go can't run in the web bundle).

A genuine UX tension surfaced building this, resolved the same direction #306 already took: there is still no write endpoint for "assign this category to a staged row" (#305/#306's own scope), so accepting a category suggestion is display-only — the card's own copy points at the real mechanism ("set it when you commit, or edit the transaction afterward"), rather than a phantom button with no network effect. An occurrence suggestion is different because a real write already exists to reach: accepting it calls `resolveImportRecordOccurrenceMatch`, exactly the same call the review screen's own deterministic Decision-column buttons reach.

That deterministic occurrence-match UI is itself a second, pre-existing gap this issue closed rather than deferred, per user-confirmed scope: [#309](https://github.com/anirudhgray/bodger/issues/309)/[#312](https://github.com/anirudhgray/bodger/issues/312) shipped `ImportRecord.occurrence_match` and `POST /api/v1/import-records/{id}/resolve-occurrence-match` on the app/CLI/HTTP surfaces, but no web UI ever read `occurrence_match` or called that endpoint — a real, if narrow, hole `Import.tsx`'s `ReviewStep` had from #309's own PR onward. #307 couldn't ship "accepting an AI-suggested occurrence match materialises it" without that action existing in the web UI first, so it's folded into this PR rather than split into a separate prerequisite: an `occurrenceMatchBadge` mirroring `duplicateBadge`'s own pattern, and Materialise/Not-this-occurrence buttons in the Decision column, alongside (never replacing) the existing duplicate-resolution buttons — the two resolve independently, per `internal/domain/importing/record.go`'s `SettleStatus` doc comment.

`web/e2e/import-suggestions.spec.ts` covers the one part of this with a real write effect a script can exercise without a live typesafe.ai key: staging a CSV row that deterministically matches a pending recurring occurrence (same account, signed amount, currency, and a token-identical description — `findOccurrenceMatch`'s own eligibility, set up by hand rather than by any model judgement) and materialising it through the review screen's own Decision-column button, against a dedicated account and rule the spec creates and archives itself, isolated from `recurring.spec.ts`'s and `smoke.spec.ts`'s own shared-account/shared-amount assertions the same way `recurring.spec.ts` isolates itself from `smoke.spec.ts`. The AI-suggestion *display* layer genuinely can't be driven end-to-end without a live key (no manual-QA network-mocking fixture exists in this project to fake one), so the present/not-configured/failed suggestion states are covered by `Import.test.tsx`'s mocked-API vitest coverage instead; the e2e spec's own not-configured assertion is that state exercised for real, since the suite never sets `BODGER_TYPESAFE_API_KEY`.
...
| M9 · Lothlórien — Recurring transactions | ✅ Complete |
| M10 · Valinor — AI-assisted suggestions | ✅ Complete |
