...
If the key isn't set, this reports that plainly and doesn't fail. If typesafe.ai itself can't be reached (or rejects the key), your staged rows are still fully listable — nothing about reviewing an import depends on this working. When some rows come back without an answer, the command says why (a rejected credential, typesafe.ai rate-limiting this instance, or typesafe.ai being unreachable) instead of just how many; `--json` carries the same reason as a short machine-readable code.
...
The web UI's **Import** page (under the sidebar's **Import & export** entry) walks through the same staged pipeline as a short wizard. "New import" starts it: choose the account first, then a file (click to browse or drag and drop) — nothing is uploaded until you continue. The next step matches each field (Date, Description, and Amount are required; Posted date, Currency, Transaction ID, and Category are optional) to a column from your file's own header row, pre-guessed from common column names but always yours to change, then "Stage for review" uploads it. The review table that follows lists every staged row with its amount and any flags — a suspected duplicate needs "Not a duplicate" or "Confirm duplicate" before you can commit; an exact duplicate is shown as already skipped, and a possible transfer is informational only. A row that matches a pending recurring occurrence gets its own flag and its own "Materialise" or "Not this occurrence" decision, next to (not instead of) a duplicate decision if it also has one — the same two-independent-reasons rule the CLI's `resolve-occurrence` command follows above. "Commit import" is disabled until every flagged row has a decision. **Import history**, below, lists every import you've started with its status; a staged or reviewed one has a "Continue review" button to pick back up where you left off, and a committed one has "Roll back" to undo it — a confirmation dialog first, since rolling back deletes the transactions it created (they keep their history, the same as deleting any other transaction).

If `BODGER_TYPESAFE_API_KEY` is set, the review table's **Suggested** column shows what typesafe.ai proposed for a row, ranked best first and always named "Suggested by typesafe.ai" — never an unattributed hint, and never a raw confidence number. A suggested category is shown for reference only: there's no button that assigns it, since accepting one means committing the import and setting the resulting transaction's category yourself, exactly as you would without a suggestion. A suggested occurrence match looks different on purpose — a dashed border, the same "not settled money yet" treatment the **Recurring** page uses for a projected occurrence — because accepting it has a real effect: its "Accept match" button calls the same materialise action as the row's own occurrence-match decision above. If the key isn't set, the column stays quietly empty with one line saying suggestions aren't set up on this instance — the rest of the screen works exactly as it does without the feature. If typesafe.ai can't be reached or rejects the key, the screen says so in plain language instead of the raw reason code, and every staged row is still fully listable and reviewable regardless.
