...
The **Analytics** screen charts your spending and income by category, your cash flow month by month, how this month compares to last, and your savings rate — the same figures `bodger report` prints on the command line, computed the same way. Pick a date range and a currency at the top; leaving the dates blank charts everything you've ever recorded. If a chart can't convert something into your chosen currency, it's listed under "Not converted" rather than silently left out of the total, with a **Refresh rates** button to fetch whatever's missing.
