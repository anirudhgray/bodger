...
The **Settings** area covers everything about your account and ledger setup, split into its own page per section with a tab strip across the top to switch between them:
...
- **Accounts** and **Categories** — each on its own page: add, rename, and archive either, and move a category under a different parent (or back to the top level). This is the same CRUD the CLI's `bodger accounts` and `bodger categories` commands expose, for whenever a browser is more convenient than a terminal.
