...
var routeTable = []route{
	...
	{
		Method: http.MethodGet, Pattern: "/api/v1/recurring-rules",
		Handler: func(h *handlers) http.HandlerFunc { return h.listRecurringRules },

		OperationID: "listRecurringRules", Summary: "List every recurring rule.",
		Description:   "Includes archived rules, the same as GET /api/v1/budgets includes archived budgets - filtering to \"current\" is left to the caller.",
		SuccessStatus: http.StatusOK, SuccessDescription: "Every recurring rule, most recently created first.",
		Response: []recurringRuleView{},
	},
	{
		Method: http.MethodPost, Pattern: "/api/v1/recurring-rules",
		Handler: func(h *handlers) http.HandlerFunc { return h.createRecurringRule },

		OperationID: "createRecurringRule", Summary: "Create a recurring rule.",
		Description:   "Creates the rule's template only - no occurrence is generated by this call.",
		SuccessStatus: http.StatusCreated, SuccessDescription: "The created rule.",
		Request: createRecurringRuleRequest{}, Response: recurringRuleView{},
		Errors: []int{http.StatusUnprocessableEntity},
	},
	{
		Method: http.MethodGet, Pattern: "/api/v1/recurring-rules/{id}",
		Handler: func(h *handlers) http.HandlerFunc { return h.getRecurringRule },

		OperationID: "getRecurringRule", Summary: "Fetch one recurring rule by ID.",
		SuccessStatus: http.StatusOK, SuccessDescription: "The rule.",
		Response: recurringRuleView{},
		Errors:   []int{http.StatusNotFound},
	},
	{
		Method: http.MethodPatch, Pattern: "/api/v1/recurring-rules/{id}",
		Handler: func(h *handlers) http.HandlerFunc { return h.patchRecurringRule },

		OperationID: "patchRecurringRule", Summary: "Update a recurring rule's amount, description, schedule, and end date.",
		Description:   `This is a full replacement of every editable field, not a partial patch: omitting "ends_on" clears it to "runs indefinitely", not to the rule's existing value. The rule's account, category, and starts-on date can't change - archive it and create a new one instead. If the schedule's own firing pattern actually changes, the rule's pending future occurrences are discarded and regenerated.`,
		SuccessStatus: http.StatusOK, SuccessDescription: "The updated rule.",
		Request: patchRecurringRuleRequest{}, Response: recurringRuleView{},
		Errors: []int{http.StatusNotFound, http.StatusUnprocessableEntity},
	},
	{
		Method: http.MethodDelete, Pattern: "/api/v1/recurring-rules/{id}",
		Handler: func(h *handlers) http.HandlerFunc { return h.archiveRecurringRule },

		OperationID: "archiveRecurringRule", Summary: "Archive a recurring rule.",
		Description:   "Stops the rule from generating any further occurrences; its history (including already materialised or skipped occurrences) remains fully queryable. Any still-pending occurrence is cancelled (removed) rather than left dangling.",
		SuccessStatus: http.StatusOK, SuccessDescription: "The archived rule.",
		Response: recurringRuleView{},
		Errors:   []int{http.StatusNotFound},
	},
	{
		Method: http.MethodGet, Pattern: "/api/v1/recurring-occurrences",
		Handler: func(h *handlers) http.HandlerFunc { return h.listScheduledOccurrences },

		OperationID: "listScheduledOccurrences", Summary: "List scheduled occurrences, optionally by rule, status, and date range.",
		Description:   "An occurrence is a projection - it never represents money that has actually moved (data-model.md §11) - until it's materialised, at which point it points at the real transaction it produced.",
		SuccessStatus: http.StatusOK, SuccessDescription: "Every matching occurrence, ascending by occurrence date.",
		Response: []scheduledOccurrenceView{},
		Errors:   []int{http.StatusNotFound, http.StatusUnprocessableEntity},
		Query: []queryParam{
			{Name: "rule_id", Description: "Only include occurrences of this rule.", Type: "string"},
			{Name: "status", Description: "Only include occurrences in this status.", Type: "string", Enum: []string{"pending", "materialised", "skipped"}},
			{Name: "from", Description: "Only include occurrences on or after this date.", Type: "string", Format: "date"},
			{Name: "to", Description: "Only include occurrences on or before this date.", Type: "string", Format: "date"},
		},
	},
	{
		Method: http.MethodPost, Pattern: "/api/v1/recurring-occurrences/{id}/materialise",
		Handler: func(h *handlers) http.HandlerFunc { return h.materialiseOccurrence },

		OperationID: "materialiseOccurrence", Summary: "Turn a pending occurrence into a real transaction.",
		Description:   "Always uses the occurrence's owning rule's current amount, account, category, and description, and the occurrence's own projected date - there is no way to override any of these for this one materialisation. Materialise with the projected fields, then PATCH the resulting transaction, to record it with a different amount or date.",
		SuccessStatus: http.StatusOK, SuccessDescription: "The transaction it produced, and the occurrence's own updated state.",
		Response: materialiseOccurrenceView{},
		Errors:   []int{http.StatusNotFound, http.StatusConflict},
	},
	{
		Method: http.MethodPost, Pattern: "/api/v1/recurring-occurrences/{id}/skip",
		Handler: func(h *handlers) http.HandlerFunc { return h.skipOccurrence },

		OperationID: "skipOccurrence", Summary: "Record that a pending occurrence's firing was deliberately not recorded.",
		Description:   "No transaction is created. Money never moved and never will for this occurrence's date.",
		SuccessStatus: http.StatusOK, SuccessDescription: "The occurrence's own updated state.",
		Response: scheduledOccurrenceView{},
		Errors:   []int{http.StatusNotFound, http.StatusConflict},
	},
	{
	...
	},
	{
		...
		OperationID: "getForecast", Summary: "Projected inflow/outflow/net from pending scheduled occurrences, bucketed by period.",
		Description:   "Reads only pending occurrences - materialised and skipped occurrences are resolved history, not a forecast - and never touches a balance, budget actual, or any other actuals-only query. \"from\"/\"to\" are both required, zero-filled across the whole range so a continuous chart has every period to plot.",
		SuccessStatus: http.StatusOK, SuccessDescription: "One point per period, ascending.",
		Response: forecastView{},
		Errors:   []int{http.StatusUnprocessableEntity},
		Query: append([]queryParam{
			{Name: "from", Description: "The inclusive start of the forecast range (required).", Type: "string", Format: "date"},
			{Name: "to", Description: "The inclusive end of the forecast range (required).", Type: "string", Format: "date"},
			{Name: "currency", Description: "Convert every figure in the result into this currency. Defaults to your configured reporting currency.", Type: "string"},
			{Name: "policy", Description: "Which conversion policy to use.", Type: "string", Enum: []string{"transaction_date", "current", "pinned"}},
			{Name: "pinned_date", Description: "The pinned date to convert at when policy is \"pinned\".", Type: "string", Format: "date"},
		}, granularityQueryParam),
	},
}
